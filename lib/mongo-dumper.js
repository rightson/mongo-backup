...
class MongoDumper {
    ...
    async dumpMonth(monthRange, progressCallback) {
        ...
        console.log(`  Documents to dump: ${totalDocs.toLocaleString()}`);

        if (totalDocs === 0) {
            console.log('  ✓ Skipping empty month');
            return { key, documents: 0, file: fullPath };
        }
        ...
        try {
            for await (const doc of cursor) {
                if (this.format === 'json') {
                    // JSONL format: one JSON object per line (more efficient for large collections)
                    writeStream.write(JSON.stringify(doc) + '\n');
                } else {
                    // For BSON format, you'd need to implement BSON serialization
                ...
                if (processedDocs % 1000 === 0) {
                    progressCallback && progressCallback(processedDocs, totalDocs);
    ...
    async run() {
        try {
            ...
            for (let i = 0; i < pendingRanges.length; i++) {
                ...
                try {
                    const result = await this.dumpMonth(range, (processed, total) => {
                        const percent = ((processed / total) * 100).toFixed(1);
                        process.stdout.write(`\r  Progress: ${processed.toLocaleString()}/${total.toLocaleString()} (${percent}%)`);
                    });
